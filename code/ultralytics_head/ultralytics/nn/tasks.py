    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, 
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose)) else self.forward(x) # OBB, YOLOESegment
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
     ...
     #           args[2] = int(max(round(min(args[2], max_channels // 2 // 32)) * width, 1) if args[2] > 1 else args[2])
      ...
      #      c2 = args[1] if args[3] else args[1] * 4
        ...
        elif m.__name__ == "OCCAPCCChannelAttention":
            c2 = ch[f]
            args = [c2]
        elif m in frozenset(
            {Detect, Segment, Pose, Detect_Efficient3DBB}
        ):  # OBB, WorldDetect, YOLOEDetect, v10Detect, YOLOESegment, ImagePoolingAttn
            args.append([ch[x] for x in f])
            if m is Segment: # or m is YOLOESegment
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, Detect_Efficient3DBB}: # YOLOEDetect, YOLOESegment, OBB
                m.legacy = legacy
