...
from ultralytics.nn.utils import attempt_load_one_weight, attempt_load_weights
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    preds = self.model(batch['img']) # Get the prediction result
                    if isinstance(preds, tuple): # If the model returns (y, x), take the first
                        preds = preds[0]
                        
                    self.loss, self.loss_items = self.criterion(preds, batch)
                 #   print("[DEBUG] preds shape:", preds.shape if isinstance(preds, torch.Tensor) else [p.shape for p in preds])
               ...
               # print("Loss shape:", self.loss.shape)
                ...
                # Backward
                self.scaler.scale(self.loss.mean()).backward()
